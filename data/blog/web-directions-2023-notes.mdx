...
conference that covers various different topics around web development. The theme for this year conference was (as I quote from their website) 
...
<Collapsible title="Notes">
    ...
    - Web is on the right path to be the building blocks for cross-platform application
        ...
        - Intel build the tech for web that’s supported by the hardware that they currently build and optimising.

    - Developer bet on the web technology
        - Web technology requires interopability - browsers
        - Mention about Progressive Web Apps that dominate the application open on the browser on PC
    ...
    - Safety and Privacy on the web native app
        - How does web native app take care of the user privacy and safety for the user?
            - App signing, action approval

    - Web API that support performance
        - [WebAssembly](https://webassembly.org/)
        - [WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API)
        - [WebGPU](https://developer.mozilla.org/en-US/docs/Web/API/WebGPU_API)
        - Web Neural Network?
        - WebCodec → Accelerating media processing - The work between Intel and Google and has been used by Zoom to accelerate the video conference call
    ...
    - AI at Scale Web Neural Network API
        ...
        - In this talk, the speaker also showcase of the performance example on how use WebNN can improve the performance of background replacement flow compare to purely on WASM
        - a low level ML API that’s built specifically to support the use case of ML/AI in the web platform
        - Features that dev need in terms of:
            - Background blury
...
<Collapsible title="Notes">
    - Challenges at Mecca for micro-frontend
        ...
        - Cross application communication
            - Custom events
                - Use lot of event listeners for diff
            - Props and callbacks?
            - Web Workers
            - Pub/Sub library
    - MicroFrontend Composition at Mecca
        - Different packages for different part of the page (micro-frontend published as a NPM Package)
        ...
        - Higher scalability and resilience is built by putting LoadBalancer at the front of Mecca reverse proxy
        - Development and deployement consistency challenge
            - Question to ask?
    ...
    Learning from Mecca microfrontend journey

    Pros

    - It probably better to have a smaller repos
    ...
    - Not ideal for a smaller projects
    - Less consistency
...
<Collapsible title="Notes">
    ...
    1. Cummulative Layout Shift (CLS)
        - Layout Jank
    ...
    3. Local Data Cache
        - Remember what use has already seen and not refetching the data that user has already saw
        - swr pattern - Use react-query
        - The speaker show a demo to build a vanilla JS SWR Implementation
        - how to persist the data
            - localStorage and sessionStorage
            - IndexedDB
            - CacheAPI
    4. Prefetching
        - Prefetch focus on data/text element like data requests or JS/CSS
        - When to pre-fetch
            - Immediately
...
<Collapsible title="Notes">
    ...
    Below are some of the strategies and factors to consider when working to improving the legacy codebase
    ...
    - Make changes in multiple batches
        - Each changes should be trivial
    ...
    - Avoid Scope Creep
        ...
        - Drop it and come back later if something taking up lot of our times
...
- Business mindset is required to grasp the overall impact of the incident to the busines
...
1. Notify and escalate
    ...
    - Does this incident needs escalation?
        - Do I need other stakeholder involve in the incident or not

1. Investigate and diagnose
    - What info is relevant towards incident resolution
    - Has the root cause identified? Do we understand the cause of the root cause?
        - Prioritise the root cause over symptoms of the issue

1. Resolve and recovery
    - Do the fastest solution to eliminating downtime without compromising system’s health and stability
    - Is there any action-items needed after resolving the issue
        - Preventing a recurring issue is the priority
2. Closure
    - Be an end-to-end communicator. Communicate with the stakeholder that’s involved or need to know that the incident has been resolved.
...
- Work methodical
- Able to ask for help - it always important to resolve the issue align with the impact that it will cause to the busines
- Sense of ownership and initiative - Monitor post incident to make sure that the issue is not continue to become the problem
- Good communicator

Some tips from the speaker on how can we be more proactive after the incident happens:
- During the on-call shifts handoffs
  - Celebrate team success of resolving the issue
...
- Update the incident runbooks if needs be
- See if there are parts that can be automated - candidates for self-remediation
- Once the issue is handled - share the knowledge to a broader engineering team 
</Collapsible>
...
<Collapsible title="Notes">
- First problem - commiting secret key to the public github repo
  - This remind me of the same incident that affecting me during my uni. time. It wasn't as much as the speakers
    demonstrate (Mine was around $4k within 80hrs), but it has shown that how this small mistake can cost you money.
    Fortunately, AWS waive that cost within my account as I've begged for mercy as I'm only student at that time.
...
Do something that make us better at what we’re doing in a secure way

Overall, the talks is presented in a theatrical way and engaging to showcase several scenarios where we can get hacked
in the real world
</Collapsible>
...
- Cross-Site scripting (XSS)
    ...
    - XSS in React
        - Improper sanitisation while outputing html - the use `dangerouslySetInnerHTML`
            - Can be solved by using the 3rd party library to sanitise the HTML output for this props
        - React escape hatch - a way for react to direct DOM manipulation
        - URLs, CSS, Markdown all can be used to run malicious code
            - Avoid take URL as an input
...
- Use CSRF Tokens as a security measures against request from the UI 
    - Some strategies atlassian has taken
      - State-changing opeartions must not use GET requests, as CSRF tokens cannot protect these.
...
### Notes
The closing keynote for the first day conference more into a stories of the journey the speaker has gone through from
multiple iterations in Web development technology and how he emphasise the important of sticking to the fundamentals
...
- Acquired different companies (3) with diff. tech. stack and UX/UI solutions
    - Different external services consumed by these diff. companies and we need to reduce the cost for it
...
- Docusaurs (first step - minimum effort)
    - Documentation for all part of engineering stakeholder
...
- Childhood stage
    ...
    - Build a template generator to speedup the creation of new application for different team
...
- Adolescence stage
    ...
    - Tooling upgradability
        - Provide a non-breaking additive changes and LTS versions drop timeline
        - Use codeshift/codemod for frontend code transformer
        - Use renovate for dependencies upgrade
        - Microservices, apss, infra use JHipster
...
Tracking customers traction by using backstage as a platform team 

- Use backstage tech. radar to measure adoption of the components
</Collapsible>
...
### Notes
In this talk, the speakers mostly talked about his experience using generative AI tools and AI coding assistant as a
part of the development workflow. He showcase an example of using the AI tools to build a calculator application. In his
demo, he wasn't just showing how he use AI tools to help building out the pieces for the Calculator, but also
understanding the concept around algorithm that's in the app itself.

Overall, there's nothing new in this talk for me personally, but it always good to see how other people utilising this
...
Edge → Serverless function that’s run close to where’s your user (consumer) call it

- Runtiime → the runtime version of the function that’s running
- WinterCG
- https://github.com/vercel/edge-runtime

Edge computing to answer the issue with data grows exponentially, to distribute the load of transferring data between different location of the consumer (users)
...
In this talk Aliaksei will walk you through the the push-based systems and how to approach that using modern front-end frameworks and libraries.

### Notes

I have a high expectation for this talk to learn something about this architecture, but it turns out the whole material
is a bit dry and when it gets into the topic that interesting enough, the speaker didn't deep dive into the topic. So,
...
<Collapsible title="Notes">
Before container queries available, the existing solution is to use the media queries to determine the different layout for different viewports
...
Media queries can also be used with range syntax 
...
And query it with `@container` query.
...
`margin-inline-start` automatically add margin that works both on LTR and RTL language
...
<Collapsible title="Notes">
This is more into an intro talk on the need of applying the tools to improve the quality and standard within the code than anything else.

I like the fact that Josh deliver the talk in the attractive and energizing but there’s not many new things that I learn from this talk.

Static Analysis

- Tooling that scrutinise your code without moving it vs. dynamic analysis - Test
    
Spectrum in the static analysis

Formatter - Linter - Types

The speaker discuss the detail of each of the spectrum above

Typescript is four things

- Language Service
    - Program that run in your editor that check for types
- Compiler
- Type Checker
    - Program that reads in files and report likely bugs/mismatch
